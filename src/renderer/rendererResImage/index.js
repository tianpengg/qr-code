...
function getViewBox(qrcode) {
  ...
  return qrcode.$options.isSpace
    ? `${-nCount / 5} ${-nCount / 5} ${nCount + (nCount / 5) * 2} ${nCount + (nCount / 5) * 2
    }`
    : `${0} ${0} ${nCount} ${nCount}`;
}
...
const RendererResImage = ({ qrcode, params }) => {
  ...
  const sBlackWidth = params[7];
  const sBlackHeight = params[8]
...
const schemaResImage = yup.object().shape({
  ...
  // 二维码点的宽度
  sBlackWidth: yup.number().default(1.02),
  // 二维码点的高度
  sBlackHeight: yup.number().default(1.02)
});
/**
 *
 * @param {*} qrcode
 ...
 * @param {Number} options.sBlackWidth 二维码点的宽度
 * @param {Number} options.sBlackHeight 二维码点的高度
 */
